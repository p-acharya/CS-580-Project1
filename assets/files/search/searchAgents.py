    ...
    def __init__(
        ...
        if 'heuristic' not in func.__code__.co_varnames:
            ...
            if heuristic in globals().keys():
                ...
                raise AttributeError(
                    heuristic + ' is not a function in searchAgents.py or '
                                'search.py.'
                )
            print(
...
def cornersHeuristic(state, problem: CornersProblem):
    ...
    "*** YOUR CODE HERE ***"
    currentPosition, visitedCorners = state
    unvisitedCorners = [problem.corners[i] for i in range(len(problem.corners))
                        if not visitedCorners[i]]
    if not unvisitedCorners:
        return 0

    # Compute the Manhattan distance to each unvisited corner and return the
    # minimum
    heuristic = min(
        util.manhattanDistance(currentPosition, corner) for corner
        in unvisitedCorners
    )
    return heuristic
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def __init__(self, gameState):
        """Stores information from the gameState.  You don't need to change
        this.
        """
